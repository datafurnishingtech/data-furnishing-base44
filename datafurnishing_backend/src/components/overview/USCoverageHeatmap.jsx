...
const legend = [
  { label: "97%+", color: "#312e81" }, { label: "93–96%", color: "#3730a3" },
  { label: "88–92%", color: "#4338ca" }, { label: "83–87%", color: "#4F46E5" },
  { label: "78–82%", color: "#6366f1" }, { label: "<78%", color: "#a5b4fc" },
];
...
export default function USCoverageHeatmap() {
  ...
  useEffect(() => {
    fetch("https://cdn.jsdelivr.net/npm/us-atlas@3/states-10m.json")
      .then((r) => r.json())
      .then((us) => setStates(feature(us, us.objects.states).features));
  }, []);
  ...
  const handleMouseMove = (e, fips) => {
    ...
    setTooltip({
      fips, pct: fipsCoverage[fips] ?? 0,
      x: (e.clientX - svgRect.left) * (WIDTH / svgRect.width),
      y: (e.clientY - svgRect.top) * (HEIGHT / svgRect.height),
    });
  };
    ...
    <div className="relative">
      <svg ref={svgRef} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full">
        {states.map((s) => {
          ...
          return (
            <path key={fips} d={pathGenerator(s)} fill={coverageToColor(pct)} stroke="white" strokeWidth={0.5}
              onMouseMove={(e) => handleMouseMove(e, fips)} onMouseLeave={() => setTooltip(null)} className="cursor-pointer hover:opacity-80 transition-opacity" />
          );
        })}
        {tooltip && (
          <g>
            <rect x={tooltip.x - 60} y={tooltip.y - 40} width={120} height={35} rx={4} fill="rgba(0,0,0,0.8)" />
            <text x={tooltip.x} y={tooltip.y - 22} textAnchor="middle" fill="white" fontSize={10} fontWeight={600}>{fipsName[tooltip.fips] ?? "Unknown"}</text>
            <text x={tooltip.x} y={tooltip.y - 10} textAnchor="middle" fill="#a5b4fc" fontSize={9}>{tooltip.pct}%</text>
          </g>
        )}
      </svg>
