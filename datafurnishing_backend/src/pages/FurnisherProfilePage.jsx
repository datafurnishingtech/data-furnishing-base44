      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1 text-[10px] text-muted-foreground/60">
                {domain && <a href={company.website_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-foreground">{domain} <ExternalLink className="w-2.5 h-2.5" /></a>}
        ...
        <div className="px-6 pb-6 space-y-4">
          ...
          <div className="bg-card rounded-lg border border-border p-4 text-[11px]">
            ...
            {company.long_description || company.short_description ? <p className="text-muted-foreground mb-3 leading-relaxed">{company.long_description || company.short_description}</p> : null}
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
              {company.headquarters_location && <div className="flex justify-between"><span className="text-muted-foreground/60">Headquarters</span><span>{company.headquarters_location}</span></div>}
              {company.country && <div className="flex justify-between"><span className="text-muted-foreground/60">Country</span><span>{company.country}</span></div>}
              {company.state && <div className="flex justify-between"><span className="text-muted-foreground/60">State</span><span>{company.state}</span></div>}
              {company.entity_type && <div className="flex justify-between"><span className="text-muted-foreground/60">Entity type</span><span>{company.entity_type}</span></div>}
              {company.detected_bureaus && <div className="flex justify-between"><span className="text-muted-foreground/60">Detected bureaus</span><span>{company.detected_bureaus}</span></div>}
              {company.reporting_relationship && <div className="flex justify-between"><span className="text-muted-foreground/60">Reporting relationship</span><span>{company.reporting_relationship}</span></div>}
              <div className="flex justify-between"><span className="text-muted-foreground/60">API available</span><span>{company.api_available ? "Yes" : "No / unknown"}</span></div>
            </div>
          </div>
