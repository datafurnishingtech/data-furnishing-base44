...
import ProductDataPoints from "@/components/furnishers/ProductDataPoints";
...
export default function FurnisherDetailPanel({ company, productCount, onClose }) {
  ...
  const companyKey = company?.company_id || company?.id;
  ...
  const openFullProfile = () => {
    if (!company) return;
    const slug = encodeURIComponent(company.slug || company.company_id || company.id);
    navigate(`/furnishers/${slug}`);
  };
  ...
  const loadProducts = async () => {
    ...
    setLoading(true);
    try {
      const { rows: prods } = await listProductsPaged({
        companyId: companyKey,
        page: 1,
        pageSize: 100,
      });
      setProducts(prods.map((p) => ({ ...p, id: p.product_id || p.id })));
      const coverage = await listProductBureauReporting(prods.map((p) => p.product_id).filter(Boolean));
      const coverageMap = {};
      for (const row of coverage) {
        const pid = row.product_id;
        coverageMap[pid] = [...(coverageMap[pid] || []), row];
      }
      ...
      <div className="bg-card rounded-lg border border-border/60 p-4 sticky top-20">
        ...
        {activeTab === "overview" && (
          <div className="space-y-3">
            ...
            <CompanyDetailSections company={company} compact />
            ...
            <div className="pt-2.5 border-t border-border/40">
              ...
              <button
                type="button"
                onClick={() => { setActiveTab("products"); if (!products.length) loadProducts(); }}
                className="flex items-center gap-1 text-[10px] text-primary/70 hover:text-primary transition-colors mt-2"
              >
            ...
            <button
              ...
              className="w-full mt-1 bg-primary text-primary-foreground text-[11px] font-medium h-7 rounded-md flex items-center justify-center gap-1.5 hover:bg-primary/90 transition-colors"
            >
