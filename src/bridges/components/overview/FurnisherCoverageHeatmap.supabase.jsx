...
export default function FurnisherCoverageHeatmap({ typeFilter = "all", selectedState, onStateSelect }) {
  ...
  const { data: stateCounts = {}, isLoading, isError: fetchError } = useQuery({
  ...
  const { data: stateCompanies = [] } = useQuery({
    queryKey: ["heatmap-state-companies", selectedState?.abbr, typeFilter],
    queryFn: () =>
      listCompaniesPaged({
        page: 1,
        pageSize: 50,
              ...
              <table className="w-full min-w-[280px]">
                ...
                <tbody>
                  {stateCompanies.map((c) => (
                    <tr key={c.id || c.company_id} className="border-b border-border/30 last:border-0 hover:bg-muted/20">
                      <td className="py-1.5">
                        <Link
                          to={`/furnishers/${encodeURIComponent(c.slug || c.company_id || c.id)}`}
                          className="flex items-center gap-2"
                        >
                          <FurnisherLogo domain={c.website_url?.replace(/^https?:\/\//, "").split("/")[0]} name={c.company_name} size="sm" />
                          <span className="text-[11px] text-foreground truncate max-w-[140px] hover:text-primary">{c.company_name}</span>
                        </Link>
                      </td>
