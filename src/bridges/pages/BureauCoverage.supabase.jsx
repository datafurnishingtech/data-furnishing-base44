...
const furnisherCoverageFallback = [
...
function abbrevBureau(b) {
  const s = String(b || "").toLowerCase();
  if (s.includes("trans")) return "TU";
  if (s.includes("equifax")) return "EQ";
  if (s.includes("experian") && !s.includes("small")) return "EX";
  if (s.includes("innovis")) return "IN";
  if (s.includes("sbfe") || s.includes("small business")) return "SB";
  return String(b).slice(0, 2).toUpperCase();
}

function coveragePct(bureaus) {
  const b = (bureaus || []).map((x) => String(x).toLowerCase());
  const hasTri = b.some((x) => x.includes("trans")) && b.some((x) => x.includes("equifax")) && b.some((x) => x.includes("experian"));
  if (hasTri) return 97;
  if (b.length >= 2) return 85;
  if (b.length === 1) return 60;
  return 0;
}
...
export default function BureauCoverage() {
  ...
  const [selectedFurnisher, setSelectedFurnisher] = useState(null);
  ...
  const { data: productCount = 0 } = useQuery({ queryKey: ["bc-products"], queryFn: getProductCount, staleTime: 60_000 });
  const { data: companyCount = 0 } = useQuery({ queryKey: ["bc-companies"], queryFn: getCompanyCount, staleTime: 60_000 });
  const { data: pageData, isLoading } = useQuery({
    queryKey: ["bc-directory", page, debouncedSearch],
    queryFn: () => listProductsPaged({ page, pageSize: 100, search: debouncedSearch, includeCompanyInSearch: true }),
    staleTime: 60_000,
  });

  const furnisherCoverage = useMemo(() => {
    const rows = pageData?.rows || [];
    if (!rows.length) return furnisherCoverageFallback;
    return rows.map((p) => {
      const c = p.company || {};
      const bureaus = [...new Set((p.bureaus_reported || []).map(abbrevBureau))];
      const score = Number(p.data_completeness_score ?? p.confidence_score ?? 0);
      return {
        name: c.company_name || "—",
        domain: c.domain || "",
        abbr: (c.company_name || "??").slice(0, 3).toUpperCase(),
        product: p.product_name || "—",
        type: (p.product_type || "other").replace(/_/g, " "),
        bureaus,
        coverage: coveragePct(p.bureaus_reported),
        confidence: score >= 70 ? "High" : score >= 40 ? "Medium" : "Low",
        trend: score >= 70 ? "up" : score >= 40 ? "right" : "down",
        reportingStatus: p.verification_status || "unknown",
      };
    });
  }, [pageData]);

  useEffect(() => {
    if (furnisherCoverage.length && !selectedFurnisher) setSelectedFurnisher(furnisherCoverage[0]);
  }, [furnisherCoverage, selectedFurnisher]);
    ...
    <div className="flex flex-col lg:flex-row gap-6 min-w-0">
      <div className="flex-1 min-w-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
          <StatCard label="Covered furnishers" value={isLoading ? "—" : companyCount.toLocaleString()} change={12.6} />
          <StatCard label="Consumer bureau cov." value={isLoading ? "—" : `${furnisherCoverage.length ? Math.round((furnisherCoverage.filter((f) => f.bureaus.includes("EX")).length / furnisherCoverage.length) * 100) : 0}%`} change={2.4} />
          <StatCard label="Business bureau cov." value={isLoading ? "—" : `${furnisherCoverage.length ? Math.round((furnisherCoverage.filter((f) => f.bureaus.includes("SB")).length / furnisherCoverage.length) * 100) : 0}%`} change={1.8} />
          <StatCard label="Verified cov. signals" value={isLoading ? "—" : productCount.toLocaleString()} change={15.3} />
          ...
          <div className="px-4 py-3 border-b border-border/50">
            ...
            <div className="flex gap-3 items-center mb-3">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search furnishers or products..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-7 text-[11px] border-border/60" />
              </div>
              <CoverageFilters 
