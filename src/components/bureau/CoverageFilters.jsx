  ...
  return (
    <div className="flex gap-2 items-center">
      ...
      <div className="relative">
        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-border/60 bg-card text-[11px] font-medium text-foreground transition-colors hover:bg-muted/50 group">
          {reportingStatus ? REPORTING_STATUS_OPTIONS.find(o => o.value === reportingStatus)?.label : "Reporting Status"}
          <ChevronDown className="w-3 h-3 text-muted-foreground/60 opacity-60 group-hover:opacity-100" />
        </button>
        <div className="absolute top-full left-0 mt-1 w-48 bg-card border border-border/60 rounded shadow-lg z-20 hidden group-hover:block">
          <button
            ...
            className="w-full text-left px-3 py-2 text-[11px] text-foreground/70 hover:bg-muted/50 hover:text-foreground transition-colors"
          >
            All statuses
          </button>
