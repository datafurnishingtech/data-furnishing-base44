    ...
    <div className="flex items-center justify-between px-4 py-2.5 border-t border-border/50">
      ...
      <div className="flex items-center gap-3">
        <span className="text-[10px] text-muted-foreground/60 tabular-nums">
          Showing <span className="text-foreground/70 font-medium">{start}–{end}</span> of <span className="text-foreground/70 font-medium">{totalItems.toLocaleString()}</span>
        </span>
        {totalPages > 1 && (
      ...
      {totalPages > 1 && (
        <div className="flex items-center gap-0.5">
          ...
          <button
            ...
            disabled={page === 1}
            className={`${btnBase} gap-0.5 px-2 ${page === 1 ? btnDisabled : btnInactive}`}
          >
          ...
          <button
            ...
            className={`${btnBase} gap-0.5 px-2 ${page === totalPages ? btnDisabled : btnInactive}`}
          >
