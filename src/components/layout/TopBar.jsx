...
export default function TopBar() {
  return (
    <header className="h-12 bg-card flex items-center justify-between px-5 sticky top-3 z-40 rounded-xl shadow-none border border-border">
      {/* Search */}
      <div className="relative w-full max-w-sm">
        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
        <Input
          placeholder="Search furnishers, products, trade data..."
          className="pl-8 pr-12 h-8 bg-background border-border text-xs"
        />
